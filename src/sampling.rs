...
use crate::{
    ...
    measurement::TrialResult,
    schedule::{MeasurementSchedule, SamplingSchedule, TrialPhase},
};

pub(crate) const TRIAL_ELAPSED_METRIC: &str = "bperf.trial.elapsed_ms";
#[cfg(test)]
...
pub(crate) enum PilotStopReason {
    Stable,
    MaximumSamples,
}
...
pub(crate) struct SamplingDecision {
...
impl SamplingDecision {
    pub(crate) fn final_samples_for(&self, workload_id: &str, engine: Engine) -> Option<u32> {
        self.strata
            .iter()
    ...
    pub(crate) fn batch_size_for(&self, workload_id: &str, engine: Engine) -> Option<u32> {
        self.strata
            .iter()
            .find(|stratum| stratum.workload_id == workload_id && stratum.engine == engine)
    ...
    pub(crate) fn pilot_samples_for(&self, workload_id: &str, engine: Engine) -> Option<u32> {
        self.strata
            .iter()
            .find(|stratum| stratum.workload_id == workload_id && stratum.engine == engine)
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct StratumDecision {
...
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct MetricEstimate {
    pub metric: String,
